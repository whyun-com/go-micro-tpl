...
var (
    ...
    regexConsul, _ = regexp.Compile("^([A-z0-9.]+):?([0-9]{1,5})?/([A-z_\\-0-9]+)$")
)
...
func (cb *consulBuilder) Build(target resolver.Target, cc resolver.ClientConn, opts resolver.BuildOptions) (resolver.Resolver, error) {
    ...
    resolver4Consul := &consulResolver{
    ...
    resolver4Consul.wg.Add(1)
    go resolver4Consul.watcher()
    return resolver4Consul, nil
